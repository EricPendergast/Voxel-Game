...
void main(){
	...
	vec4 sun = vec4(254.0/255.0,244.0/255.0,82.0/255.0,1.0);
	float atmosDist = distanceThroughAtmosphere(playerPos, fragPos, planetPos, planetRadius);
	vec4 atmosphere = vec4(92.0/255.0,152.0/255.0,215.0/255.0,1.0);
	vec4 aTop = vec4(92.0/255.0,152.0/255.0,215.0/255.0,1.0);
	vec4 aBot = vec4(1,1,1,1);
	atmosDist /= 50.0;
	atmosDist = min(1, atmosDist);
	atmosDist = max(0, atmosDist);
	aTop.a = atmosDist;
	aBot.a = 1.0-atmosDist;

	//atmosphere.a = atmosDist/50;
	atmosphere = blend(aTop,aBot);
	sun.a = fragToSunDist < 0.0 ? 0.0 : sun.a;
	sun.a /= max(fragToSunDist-sunRadius,0.0)*1.0;
	sun.a = min(1.0,max(0.0,sun.a-.5));
...
vec4 blend(vec4 a, vec4 b){
	...
	vec4 ret = vec4(0.0, 0.0, 0.0, 1.0 - (1.0-a.a)*(1.0-b.a));
	
	ret.r = a.r*a.a/ret.a + b.r*b.a*(1.0-a.a)/ret.a;
	ret.g = a.g*a.a/ret.a + b.g*b.a*(1.0-a.a)/ret.a;
	ret.b = a.b*a.a/ret.a + b.b*b.a*(1.0-a.a)/ret.a;
	return ret;
...
float distanceThroughAtmosphere(vec3 p1, vec3 v2, vec3 s3, float r){
	...
	float b = 2.0*p1.x*v2.x - 2.0*v2.x*s3.x  +  2.0*p1.y*v2.y - 2.0*v2.y*s3.y  +  2.0*p1.z*v2.z - 2.0*v2.z*s3.z ;
	float c = p1.x*p1.x - 2.0*p1.x*s3.x + s3.x*s3.x  +  p1.y*p1.y - 2.0*p1.y*s3.y + s3.y*s3.y  +  p1.z*p1.z - 2.0*p1.z*s3.z + s3.z*s3.z - r*r;
	
	float d = sqrt(b*b - 4.0*a*c);
	
	float sol1 = (-b+d)/(2.0*a);
	float sol2 = (-b-d)/(2.0*a);
	if(min(sol1,sol2) > 0.0)
...
float getDistance(vec3 p1, vec3 p2, vec3 v3){
	...
	float dist = sqrt(pow(p2.x+v3.x*t-p1.x,2.0) + pow(p2.y+v3.y*t-p1.y,2.0) + pow(p2.z+v3.z*t-p1.z,2.0));
	
	return dist*(t > 0.0 ? 1.0 : -1.0);
}
float getWavelength(float hue){
	return 400.0 + 360.0 - hue;
}
...
float getHue(vec4 color){
	...
	return mod(60.0*(2.0*float(max) + (color[int(mod(float(max)+1.0, 3.0))]-color[int(mod(float(max)+2.0, 3.0))])/(float(color[max])-float(color[min])))+360.0, 360.0);
}
